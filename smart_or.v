...
`is_merge_or {A: Type} {tc: comparable A} (r: regex A) : Prop`
...
```
merge_or_is_correct: forall {A: Type} {tc: comparable A} 
...
*)
Definition smart_or {A: Type} {tc: comparable A} (r s: regex A) : regex A :=
...
*)
Fixpoint merge_or {A: Type} {tc: comparable A} (r s: regex A) : regex A :=
...
*)
Theorem merge_or_step: forall {A: Type} {tc: comparable A} (r s: regex A),
...
Fixpoint merged_or {A: Type} {tc: comparable A} (r: regex A) : Prop :=
  match r with
  | or s t =>
    match s with
    | or _ _ => False
...
Theorem merged_or_upholds: forall {A: Type} {tc: comparable A} (r s: regex A) (mr: merged_or r) (ms: merged_or s),
...
```
*)
Fixpoint to_list_or {A: Type} {tc: comparable A} (r: regex A) : list (regex A) :=
  match r with
...
*)
Theorem to_list_or__to_tree_or__is_id: forall {A: Type} {tc: comparable A} (r: regex A) (xs: list A),
...
Program Fixpoint merge_or_program {A: Type} {tc: comparable A} (r s: regex A) {measure ((size r)+(size s))} : regex A :=
  match r with
  | or r_1 r_next =>
    match s with
    | or s_1 s_next =>
